...
@section('pageHeading')
  @if (!empty($pageHeading))
    {{ $pageHeading->customer_forget_password_page_title ?? __('Forget Password') }}
  @else
    {{ __('Forget Password') }}
  @endif
    ...
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-4">
          ...
          @endif
          <form id="login-form" name="login_form" class="login-form" action="{{ route('customer.forget.mail') }}"
            method="POST">
            @csrf
            
            <div class="form-group mt-3 d-flex justify-content-between mb-0" style="font-size: 14px">
                <p>{{ __('Remembered Password') . '?' }} <a class="text-info" href="{{ route('customer.login') }}" style="color: #f8cabd !important">{{ __('Login Now') }}</a></p>
              </div>
