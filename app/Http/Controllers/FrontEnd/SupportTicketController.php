...
class SupportTicketController extends Controller
{
  ...
  public function store(Request $request)
  {
    $request->validate([
      ...
      'attachment' => $request->hasFile('attachment') ? 'mimes:jpg,jpeg,png,pdf,zip|max:5000' : ''
    ]);
  ...
  public function reply(Request $request, $id)
  {
    ...
    $allowedExts = array('jpg,jpeg,png,pdf,zip');
    $rules = [
      'reply' => 'required',
      'file' => [
        function ($attribute, $value, $fail) use ($file, $allowedExts) {
          ...
          if (!in_array($ext, $allowedExts)) {
            return $fail("Only jpg,jpeg,png,pdf,zip file supported");
          }
        },
        'max:5000'
      ],
    ...
    $messages = [
      'file.max' => ' file may not be greater than 5 MB',
    ];
